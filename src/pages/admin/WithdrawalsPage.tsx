...
const WithdrawalsPage = () => {
  ...
  // Helper function to get investor by name
  ...
  const openWithdrawalForm = (investor: any) => {
    setSelectedInvestorForWithdrawal(investor);
  ...
  const columns = [
        ...
        return (
          <div className="space-y-2">
            ...
            {bankInfo?.accountNumber && (
              ...
              <WithdrawalFlagIndicator withdrawalId={row.id} />
            </div>
          </div>
        );
      }
    ...
    },
    {
      ...
      render: (_: any, row: any) => {
        return (
          <div className="space-y-2">
            {/* Flag withdrawal button - Available for all users */}
            <Button
              size="sm"
              variant="outline"
              onClick={() => {
            ...
            {row.status !== 'Pending' && row.status !== 'Approved' && row.status !== 'Credited' && (
